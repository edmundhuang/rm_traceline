...
uint8_t sonic_id;
sonic_sensor_t sonic_val[4];//定义超声波传感器模块相关的结构体变量
...
void sonic_mesg_handler(uint8_t send_id, sonic_sensor_t    *data)
{
  ...
  switch (send_id)
  {
    case 1:
      ...
      sonic_task(send_id, sonic_val[0].distance);
    break;

    case 2:
      memcpy(sonic_val+1, data, sizeof(sonic_sensor_t));
      sonic_task(send_id, sonic_val[1].distance);
    break;

    case 3:
      memcpy(sonic_val+2, data, sizeof(sonic_sensor_t));
      sonic_task(send_id, sonic_val[2].distance);
    break;

    case 4:
      memcpy(sonic_val+3, data, sizeof(sonic_sensor_t));
      sonic_task(send_id, sonic_val[3].distance);
    break;
  }
}
...
//void safeRegion(int16_t  distance)
//{
//
//		if(distance<200)
//		{
